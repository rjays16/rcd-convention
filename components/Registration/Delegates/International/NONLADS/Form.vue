<template>
    <section id="international-non-lads-registration">
        <div class="container my-4">
            <div class="row justify-content-center">
                <div class="col-lg-8 col-md-12">
                    <div class="text-center">
                        <h1 class="avenir-medium fw-bold">
                            {{ form_title }}
                        </h1>
                    </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-12 mb-3">
                              <label class="text mb-2">Confirm Password*</label>
                                <el-input type="password" placeholder="" v-model="$v.form.confirm_password.$model" autocomplete="new-password" show-password />
                        ...
                        <div class="form-group row">
                            <div class="col-12 mb-3">
                              <label class="text mb-2">Country</label>
                                <el-select v-model="$v.form.country.$model" class="w-100" placeholder="Country" autocomplete="off"
                                    v-if="form.type === 2">
                                    <el-option
                                      v-for="option in specific_countries"
                                        :key="option.id"
                                        :label="option.name"
                                        :value="option.name" />
                                </el-select>

                              <el-select v-model="$v.form.country.$model" class="w-100" placeholder="Country" autocomplete="off" v-else>
                                <el-option
                                  v-for="option in customed_countries"
                                  :key="option.id"
                                  :label="option.name"
                                  :value="option.name"
                                />
        ...
        <div class="container my-5">
            <div class="row text-center">
                <div class="inline-block avenir-medium">
                    <button class="btn btn-blue py-2 px-5 corner-22px fs-5 me-3" @click="$parent.goToStep(4)">
                        Back
                    </button>
...
export default {
    props: ['countries', 'specific_countries', 'customed_countries', 'GET_TYPE'],
    computed: {
        ...mapState({
		...
		return {
			form: {
                ...
                type: this.$store.state.registration.type,
                 country: this.$store.state.registration.type === 2? "Philippines" : "United States of America",
                role: this.$store.state.options.registerable_role
            },
            ...
            registration_string: "registration/registerDelegateWalkIn",
    ...
    validations: {
        form: {
            ...
            resident_certificate: {
                required: requiredIf(function() {
					return this.form.type == 4 /* require the certificate for International Delegate type */
				}),
            },
        }
    },
    methods: {
		...
		recheckInputs() { // this is used for validation
            if(this.form.resident_certificate === null && this.form.type === 4) {
                ...
                if(!this.form.has_agreed) { // check if the user has already agreed
                    ...
                    if(this.form.type == 4) {
                        this.registration_string = "registration/registerDelegateExisting"
                    }
                    this.register()
                }
            }
        ...
        register() {
			...
			this.$store.dispatch(this.registration_string, this.form)
            .then(res => {
        ...
        setRegistrationInfo(){
            // console.log('setRegistrationInfo',)
...
#international-delegates-registration {
    line-height: 18px;
    font-size: 13px;
    .header {
