<template>
    <section>
        ...
        <el-dialog title="Brochures" :visible.sync="showBrochureModal" id="brochures">
            <div v-for="(brochure, index) in brochures" :key="index" class="resource-list">
                <div class="list-name">{{ 'Brochure ' + (index + 1) }}</div>
                <div class="list-view">
                    <img src="/assets/images/blue-pdf.png" alt="">
                    <button class="view-pdf text-uppercase" @click="viewPDF(brochure)">
                        View PDF
                    </button>
                </div>
            </div>
        ...
        <el-dialog :visible.sync="showInnerBrochure" class="inner-modal" id="brochure_dialog" append-to-body>
        ...
        <el-dialog title="Videos" :visible.sync="showVideoModal">
            <div v-for="(video, index) in videos" :key="index" class="resource-list">
                <div class="list-name">{{ 'Video ' + (index + 1) }}</div>
                <div class="list-view">
                    <img src="/assets/images/blue-video.png" alt="">
                    <button @click="viewVideo(video)" class="view-pdf text-uppercase">
                        Watch Video
                    </button>
                </div>
            </div>
    ...
    methods: {
        viewVideo(video) {
            ...
            if(video) {
                ...
                this.add_visit_company_asset_view(video.id)
                // this.name = "Video No." + video.id
                // this.update_name_asset(this.name)
            } else {
            ...
            if(pdf) {
                ...
                let brochure_viewer_timer = setInterval(function () {
                    if(brochure_element) {
                        ...
                        PDFObject.embed(pdf.url, "#brochure_viewer", options)
                        this.brochure_isLoading = false
                        clearInterval(brochure_viewer_timer)
                    }
                }, 500);
                this.add_visit_company_asset_view(pdf.id)
                // this.name = "Brochure No. " + pdf.id
                // this.update_name_asset(this.name)
            } else {
                this.$message.error("The PDF url for the brochure was not found.")
            }
        ...
        viewCatalog(catalog) {
            console.log('nnn',catalog)
            this.catalog_isLoading = true

            if(catalog) {
                var url_parts = catalog.url.split('/');
        ...
        add_visit_company_profile_view() {
            this.$store.dispatch("sponsors/updateCompanyProfile", this.sponsor.id)
            .then(res => {
                // console.log('company-profile', res.data.message)
            })
            .catch((err) => {
                console.error(err)
        	});
        },
        add_visit_company_asset_view(asset_id) {
            this.$store.dispatch("sponsors/updateVisitVexBooth", {'sponsor_id': this.sponsor.id, 'sponsor_asset_id': asset_id })
            .then(res => {
                // console.log('company-asset', res.data.message)
            })
            .catch((err) => {
                console.error(err)
        	});
        },
      // update_name_asset(asset_name) {
      //   this.$store.dispatch("sponsors/updateName", {'sponsor_id': this.sponsor.id, 'name': asset_name })
      //     .then(res => {
      //     })
      //     .catch((err) => {
      //       console.error(err)
...
::v-deep .el-dialog {
    ...
    .el-dialog__header {
        ...
        .el-dialog__headerbtn {
            ...
            top: 9px;
            right: 9px;
            background: url('/assets/images/white-close.png') no-repeat center center;
            i {
                display: none;
            }
        }
    ...
    &::v-deep .el-dialog {
        ...
        .el-dialog__body {
            ...
            .resource-download {
                ...
                button {
                    border: none;
                    background: url("/assets/images/arrow-download.png") no-repeat right center transparent;
...
.resources {
    ...
    ul {
        ...
        li {
            ...
            &.company-profile {
                background: url('/assets/images/vcc/ant-design_profile-outlined.png') no-repeat left center;
            }
            &.download-brochures {
                background: url('/assets/images/vcc/teenyicons_pdf-outline.png') no-repeat left center;
            }
            &.watch-videos {
                background: url('/assets/images/vcc/et_video-booth.png') no-repeat left center;
            }
            &.products-catalog {
                background: url('/assets/images/vcc/uit_create-dashboard.png') no-repeat left center;
            }
        }
    }
}
