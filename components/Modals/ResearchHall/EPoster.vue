<template>
  <section>
    <div class="modal fade" id="e_poster" tabindex="-1" role="dialog" aria-labelledby="leftModalLabel" aria-hidden="true">
            ...
            <div class="container-fluid d-flex h-100">
              ...
              <div class="col-md-6 col-sm-12 h-100">
                <div class="heading">
                  ...
                  <h1 class="text-white text-uppercase text-center inter-bold">e-poster</h1>
                  ...
                  <div>
                    <div class="authors" v-for="(abstract, id) in abstracts" :key="id">
                      <div v-if="category === abstract.abstract_category">
                        ...
                        <a href="#" type="button" class="pdf text-uppercase text-white inter-regular" data-toggle="modal" @click="openModalEposter(abstract.id)">view paper
                          <img src="/assets/images/vcc/pdf_icon.png"></a>
                        <hr class="bg-white">
                        </div>
                      </div>
                      <div v-else></div>
    ...
    <ModalsResearchHallPDFPreview ref="PreviewModalPdf"/>
  </section>
</template>
<script>
import $ from 'jquery';
export default {
  ...
  methods: {
    get_Eposter() {
      this.$store.dispatch("abstracts/getAllEposterAbstracts")
        .then(res => {
    ...
    preview_pdf(){
      this.$refs.PreviewModalPdf.preview_pdf(this.abstracts)
      $('#pdf').modal('show');
    },
    openModalEposter(id){
      this.$refs.PreviewModalPdf.openModal(id, true);
    }
  },
  mounted(){
    this.get_Eposter()
...
#e_poster .modal-dialog-slideout {
  min-height: 50%;
  margin: 0 auto 0 0;
  background: #fff;
}
#e_poster.modal.fade:not(.show) .modal-dialog.modal-dialog-slideout {
  -webkit-transform: translate(50%,0)scale(1);
